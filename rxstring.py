...
#MESH SPARQL Query for finding terms
                   ...
                   MESHheaders = {"User-Agent": "rxstring/1.0", "From": "tyler.ostapyk@umanitoba.ca"}
...
#Section for truncating and phrase searching single source. Noted out for now.                  							   
                               ...
                               rxaltsourcestring = ' OR '.join(rxalt)
             ...
             if WikidataSearch=="on":
                 ...
                 if results==[]:
                     ...
                     combinedsourcestring = " OR ".join(results)
               ...
               if efilename=="":
                 ...
                 for etrees in target.find_next_siblings():
                            if etrees.name=="div":
                                          ...
                                          EmtreeTermsSource = (EmtreeTerms.split("\n")[:-3])
                                          EmtreeTermsSource = [x for x in EmtreeTermsSource if x]
                                          EmtreeTermsSource = [x.strip() for x in EmtreeTermsSource]
                                          EmtreeTermsSource = sorted(EmtreeTermsSource)
                                          ...
                                          EmtreeTermsSourceString = " OR ".join(EmtreeTermsSource)
...
#If phrase search is on put quotation marks around each separate term when joining
                 if PhraseSearch=="on":
                     if TruncationSymbol=="on":
                            ...
                            FormattedPage = "<html><head><title>RxString Results</title><meta name='viewport' content='width=device-width, initial-scale=1.0'><style>h2 {font-size:1.4em;} .resultslogo {float:left;} .responsecontent {margin-left:120px; margin-top:25px;} @media only screen and (max-width: 600px) {.resultslogo {grid-area: 1 / span 3; float:none; height:auto;} .responsecontent {grid-area: 2 / span 3; margin-left:auto; margin-top:auto} .resultsfooter {grid-area: 3 / span 3;} .resultslogoimage {display:block; margin-left:auto; margin-right: auto;}</style>" + googleanalytics + "</head><body>" + "<div class='resultslogo' style='margin-top:10px'><img class='resultslogoimage' width='100px' src='https://raw.githubusercontent.com/Twostap/rxstring/refs/heads/main/rxstringcropped.png'/></div><div id='responsecontent' class='responsecontent' style='font-family:arial; font-size:.8em'><h2>Query Responses</h2>" + completeintro +  "<br><br>" +  "<form><input type='button' style='color: white; background-color: #01789f; border:1px solid black; padding:4px 10px' value='New search' onclick='history.go(-1)'></form>" + "<h2>Search string</h2>" + '"' + dedupedstring + '*"' + "</div><div align='right' style='margin-right:10px; margin-top:10px;'><a style='font-size:.8em; text-decoration: none;' onclick='downloadcsv()' href='javascript:void(0);'>Download terms list (.csv)</a>"
                     elif TruncationSecond=="on":
                            TwoWordList = []

...
#Check if search term was entered into the HTML form, if not then output notice asking them to enter a search again
       if drug=="":
            ...
            FormattedPage = "<html><head><meta name='viewport' content='width=device-width, initial-scale=1.0'><style>
.resultslogo {float:left}
.resultslogoimage {margin-right:20px}

@media only screen and (max-width: 600px) {
  .resultslogo {grid-area: 1 / span 3; float:none}
  .resultscontent {grid-area: 2 / span 3;}
  .resultsfooter {grid-area: 3 / span 3;}
  .resultslogoimage {display:block; margin-left:auto; margin-right: auto;
}
</style></head><body>" + "<div class='resultslogo' style='margin-top:10px'><img class='resultslogoimage' width='100px' src='https://raw.githubusercontent.com/Twostap/rxstring/refs/heads/main/rxstringcropped.png'/></div><div><h2>Query Responses</h2></div>" + completestring + "<br><br>" +  "<form><input type='button' value='New search' onclick='window.history.go(-1)'></form>" + "</div><div style='margin-top: auto'><p style='font-size:.8em;'>© Copyright 2025 Tyler Ostapyk</p></div>"
...
#If phrase search is on put quotation marks around each separate term when joining
                 if PhraseSearch=="on":
                     ...
                     FormattedPage = "<html><head><meta name='viewport' content='width=device-width, initial-scale=1.0'></head><style>
.resultslogo {float:left}
.resultslogoimage {margin-right:20px}
